...
export async function fetchList(
  ...
  const total = (await model.aggregate([...pipeline, { $count: 'total' }]))?.[0]?.total
  ...
  if (!countResultOnly) {
    ...
    items = await (pipeline?.length ? model.aggregate(pipeline).exec() : model.find())
  }
...
export async function fetchItem(
  ...
  // Get query params
  const { searchParams } = new URL(request.url)
  const id = customId || searchParams.get('id')
